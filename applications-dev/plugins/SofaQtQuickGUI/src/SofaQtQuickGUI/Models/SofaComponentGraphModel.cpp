...
SofaComponentGraphModel::SofaComponentGraphModel(std::string name)
    : NodeDataModel()
    , debugNodeGraph(true)
...
void SofaComponentGraphModel::parseSofaObjectData()
{
    if (m_SofaObject == nullptr)
    {
...
QtNodes::PortIndex SofaComponentGraphModel::getDataInputId(const QString& dataName)
{
    int cpt = 0;
...
unsigned int SofaComponentGraphModel::nPorts(PortType portType) const
{
    return m_data.size();
}

NodeDataType SofaComponentGraphModel::dataType(PortType portType, PortIndex portIndex) const
{
...
std::shared_ptr<NodeData> SofaComponentGraphModel::outData(PortIndex port)
{
...
void SofaComponentGraphModel::setInData(std::shared_ptr<NodeData> data, int port)
{
